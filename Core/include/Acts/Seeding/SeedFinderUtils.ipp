...
using isSignatureCompatible =
    decltype(std::declval<callable_t&>() = std::declval<external_spacepoint_t>());
}
...
LinCircle transformCoordinates(
  ...
  using member_ptr_type = std::array<float, 6>(*)(const external_spacepoint_t&);

  static_assert(Concepts::is_detected<SeedFinderUtils::isSignatureCompatible, member_ptr_type, decltype(extractFunction)>::value,
  "Callable given does not correspond exactly to required call signature -> std::array<float, 6>(const external_spacepoint_t&)");
