...
class CandidatesForSpM {
 ...
 private:
  ...
  void bubbledw(std::vector<value_type>& storage, std::size_t n, std::size_t actual_size);

  void addToCollection(std::vector<value_type>& storage, sp_type& SpB, sp_type& SpT,
                       float weight, float zOrigin, bool isQuality);
  void insertToCollection(std::vector<value_type>& storage, sp_type& SpB, sp_type& SpT,
                          float weight, float zOrigin, bool isQuality);
